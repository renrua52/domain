...
**题解**. 签到题。观察到答案是第一个 L 的序号，这是因为球不可能传给 L 右边的人。

### B. Right Maximum

**题面**. 给定一个整数序列，每步找到其中最大元素（如有多个则取最右边的），删去它及其右边所有数。序列变空需要的步数。
...
> 对于原无向图中从 $v$ 开始的任意一条道路（可以包含环）$(v, v_1, \dots, v_k)$，有 $(v,v_1),(v_2,v_1),(v_2,v_3),(v_4,v_3),\dots$ 是新的有向图中的边。称这样的道路为 alternating path。
...
一个无向图是[二分图](https://oi-wiki.org/graph/bi-graph/)，如果可以将其节点分为两个子集 $X,Y$，使得边仅存在于两子集之间（而不是子集内部）。换言之，可以将图的节点染成两种颜色（$2$-着色），使得相邻节点的颜色不同。可以证明，这等价于图中不存在长度为奇数的环（[证明](https://oi-wiki.org/graph/bi-graph/#%E5%88%BB%E7%94%BB)）。一个例子如下图：
